...
class TblpenjualanModel extends Model
{
    ...
    protected $table = ! portfolio-only !;
    protected $primaryKey = ! portfolio-only !;
    protected $returnType = 'object';
    protected $useSoftDeletes = false;
    protected $allowedFields = [! portfolio-only !];
    ...
    public function getTransaksiTanggal($transaksi_id = false)
    {
        if ($transaksi_id == false) {
            return null;
        } else {
            return $this->select('tanggal')->where(['transaksi_id' => $transaksi_id])->first();
        }
    }

    public function getHargaFinal($transaksi_id = false)
    {
        if ($transaksi_id == false) {
            return null;
        } else {
            return $this->select('harga_final')->where(['transaksi_id' => $transaksi_id])->first();
        }
    }
}
