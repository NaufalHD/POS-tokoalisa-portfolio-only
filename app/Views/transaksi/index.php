...
<script>
    ...
    $(function() {
        ...
        var table = $('#cart_table').removeAttr('width').DataTable({
            ...
            "responsive": true,
            "ajax": {
                "url": '<?php echo base_url("/transaksi/! portfolio-only !") ?>',
                "type": "POST",
                "dataType": "json",
                async: "true"
            }
    ...
    $(document).on('click', '#! portfolio-only !', function() {
    ...
    function get_cart_qty(bstock_id) {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('transaksi/! portfolio-only !') ?>',
            data: {
                'bstock_id': bstock_id
            },
            dataType: 'json',
    ...
    $(document).on('click', '#! portfolio-only !', function() {
        ...
        if (bstock_id == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= site_url('transaksi/! portfolio-only !') ?>',
                data: {
                    // 'add_cart': true,
                    'bstock_id': bstock_id,
                    'harga': stok_harga,
    ...
    $(document).on('keyup mouseup', '#diskon, #! portfolio-only ! portfolio-only ! function() {
        calculate()
    })
    ...
    $(document).on('click', '#! portfolio-only !', function() {
        ...
        if (pelanggan_nama == null || pelanggan_nama == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= site_url('pelanggan/! portfolio-only !') ?>',
                data: {
                ...
                success: function(result) {
                    if (result.success) {
                        ...
                        $('#! portfolio-only !').DataTable().ajax.reload(null, false).draw(false);
    ...
    $(document).on('click', '#! portfolio-only !', function() {
        ...
        var nm_kostumer = $('#! portfolio-only !').find(':selected').data('pelanggan_nama')
        var nm_kostumer_id = $('#! portfolio-only !').find(':selected').data('pelanggan_id')
        ...
        if (total_harga < 1) {
                            ...
                            $.ajax({
                                ...
                                success: function(result) {
                                    if (result.success) {
                                        ...
                                        $.ajax({
                                            type: 'POST',
                                            url: '<?= site_url('transaksi/proses ! portfolio-only !') ?>',
                                            dataType: 'json',
                                            data: {
                                        ...
                                        $('#modal-! portfolio-only !ah-pelanggan').modal('hide')
                                        clear_input_form()
                                        // window.location = "pelanggan";
                                    } else {
                                        alert('Gagal menambahkan pelanggan')
            ...
            // } else if (1>0) {
            // alert('bro')
        } else if (kembalian < 0 && ajukan_hutang == true && nm_kostumer != "! portfolio-only !") {
            // BELUM SELESAI
            if (confirm('Ajukan hutang baru pada pelanggan: ' + nm_kostumer + " ? ")) {
                console.log('! portfolio-only ! = ' + nm_kostumer_id);
                if (confirm('Yakin proses transaksi ini ?')) {
                    $.ajax({
                        type: 'POST',
